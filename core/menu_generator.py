...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/menu_generator.log"), logging.StreamHandler()],
)
...
class MenuGenerator:
    def __init__(
        ...
        if household_id:
            # B61 (2026-07-09): these used to be computed eagerly here via
            # recipes_db_file()/menu_file(), which call household_dir() -
            # a function with a real side effect (creates
            # data/households/<id>/ and copies seed files into it) - meaning
            # every single menu generation created a household directory on
            # disk, even though these paths are only ever actually read/
            # written in the file-fallback branches below, which only run if
            # the DB itself fails. Left unset here; computed lazily, only if
            # a DB failure genuinely reaches one of those fallback branches.
            self.recipes_imported_file = None
            self.menu_output_file = None
        else:
    ...
    def load_recipes(self) -> bool:
        ...
        if self.household_id:
            try:
                ...
                try:
                    household = (
                        db.query(Household)
                        .filter(Household.id == self.household_id)
                        .first()
                    )
                    if household and household.recipes_db:
                        all_recipes.extend(household.recipes_db)
                        logger.info(
                            f"Loaded {len(household.recipes_db)} recipes from database"
                        )
                finally:
                    db.close()
            except Exception as e:
                logger.error(
                    f"Failed to load recipes from database: {e}, trying file fallback"
                )
                # B61 (2026-07-09): computed lazily here, not in __init__ -
                # recipes_db_file() calls household_dir(), which creates the
                # household's directory as a side effect. Only worth paying
                # that cost on this genuine last-resort path, not on every
                # generation's happy path.
        ...
        has_favorites = any(
            ...
            if cat.lower() in ("favorites", "favoritter")
        )
        other_categories = [
            c
            for c in self.selected_categories
            if c.lower() not in ("favorites", "favoritter")
        ]
        ...
        if has_favorites and not other_categories:
            if not self.favorite_recipe_ids:
                logger.info(
                    "Favorites selected but no recipes marked as favorites - returning empty"
                )
                return True
            favorite_recipes = [
                r for r in all_recipes if r.get("id") in self.favorite_recipe_ids
            ]
            seen_ids = set()
            for recipe in favorite_recipes:
                ...
                for field in ("title", "subtitle", "description", "comment"):
                    ...
                    if isinstance(val, dict):
                        recipe_copy[f"_filter_{field}"] = (
                            val.get("en") or val.get("no") or ""
                        )
        ...
        selected_lower_without_quick = [
            c for c in selected_lower if c != "quick dinners"
        ]
        ...
        for recipe in all_recipes:
            ...
            if (
                ...
                or category in selected_lower_without_quick
                or (wants_quick_dinners and is_quick)
            ):
        ...
        logger.info(
            f"Loaded {len(self.recipes_db)} recipes (filtered by {self.selected_categories})"
        )

        if len(self.recipes_db) == 0 and self.selected_categories:
            logger.warning(
                f"No recipes found for selected categories: {self.selected_categories}"
            )
    ...
    def filter_recipes(self) -> int:
        ...
        for recipe in self.recipes_db:
            ...
            if not subtitle and isinstance(recipe.get("subtitle"), dict):
                subtitle = (
                    recipe["subtitle"].get("en") or recipe["subtitle"].get("no") or ""
                )
            elif not subtitle:
            ...
            if not description and isinstance(recipe.get("description"), dict):
                description = (
                    recipe["description"].get("en")
                    or recipe["description"].get("no")
                    or ""
                )
            ...
            if (
                self.contains_orange(title)
                or self.contains_orange(subtitle)
                or self.contains_orange(description)
            ):
        ...
        logger.info(
            f"Filtered recipes: {len(self.filtered_recipes)} (removed {len(self.recipes_db) - len(self.filtered_recipes)} with orange)"
        )
        return len(self.filtered_recipes)

    def get_protein_type(
        self, recipe_title: str, recipe_subtitle: str = "", category: str = ""
    ) -> str:
    ...
    def generate_menu(self, num_dinners: int = 6) -> Dict:
        ...
        for i in range(num_dinners):
            ...
            for recipe in available_recipes:
                ...
                protein = self.get_protein_type(
                    title_str, subtitle_str, recipe.get("category", "")
                )
            ...
            if not best_recipe:
                ...
                if isinstance(subtitle, dict):
                    ...
                    subtitle_str = subtitle or ""
                best_protein = self.get_protein_type(
                    title_str, subtitle_str, best_recipe.get("category", "")
                )
        ...
        shopping_list = self.deduplicator.deduplicate_from_recipes(recipe_ids)[
            "shopping_list"
        ]
        ...
        for i, recipe in enumerate(selected_recipes):
            ...
            protein_type = self.get_protein_type(
                protein_title, protein_subtitle, recipe.get("category", "")
            )
            dinners.append(
                {
                    "day": DAYS[i],
            ...
            try:
                ...
                try:
                    ...
                    if household:
                        ...
                        db.commit()
                        logger.info(
                            f"Menu saved to database for household {self.household_id}"
                        )
                        return True
                finally:
                    db.close()
                ...
                logger.error(
                    f"Failed to save menu to database: {e}, falling back to file"
                )

        # Fallback to file-based storage. Computed lazily (B61, 2026-07-09) -
        # see the matching comment in load_recipes() above.
        if self.household_id:
            from core.household_paths import menu_file

            menu_output_file = menu_file(self.household_id)
        else:
            menu_output_file = self.menu_output_file

        menu_output_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def print_menu(self, menu: Dict) -> None:
        ...
        for dinner in menu["dinners"]:
            ...
            if isinstance(title, dict):
                ...
                title_str = title or ""
            logger.info(
                f"{dinner['day']:12} | {title_str:50} | {dinner['time_minutes']} min | {dinner['difficulty']}"
            )
        ...
        for category, items in shopping_list.items():
            if items:
                ...
                for item in items:
                    logger.info(
                        f"  ☐ {item['ingredient']:30} | {item['quantity']} {item['unit']}"
                    )
                logger.info("")
