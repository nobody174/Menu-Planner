...
class IngredientDeduplicator:
    ...
    def load_pantry_staples(self) -> List[str]:
        if PANTRY_FILE.exists():
            with open(PANTRY_FILE, "r", encoding="utf-8") as f:
                ...
                logger.info(
                    f"Loaded {len(staples)} pantry staples (EN: {len(en_staples)}, NO: {len(no_staples)})"
                )
                return staples
        else:
    ...
    def fuzzy_match(
        self, ingredient: str, reference_list: List[str]
    ) -> Tuple[str, int]:
    ...
    def is_pantry_staple(self, ingredient_name: str) -> bool:
        ...
        if ingredient_clean in self.pantry_staples:
            logger.info(
                f"✓ MATCH (cleaned): '{ingredient_name}' → '{ingredient_clean}' (pantry staple)"
            )
            return True
        ...
        for part in parts:
            ...
            if part_clean in self.pantry_staples:
                logger.info(
                    f"✓ PARTIAL MATCH: '{ingredient_name}' contains '{part_clean}' (pantry staple)"
                )
                return True
        ...
        for staple in self.pantry_staples:
            ...
            if score >= 70:
                logger.info(
                    f"✓ FUZZY MATCH: '{ingredient_name}' → '{staple}' (score: {score})"
                )
                return True
    ...
    def deduplicate_ingredients(self, ingredient_list: List[Dict]) -> List[Dict]:
        ingredient_groups = defaultdict(
            lambda: {"quantities": [], "units": [], "allergens": set()}
        )
        logger.info(
            f"deduplicate_ingredients called with {len(ingredient_list)} ingredients"
        )
        ...
        for ingredient in ingredient_list:
            ...
            if best_score < 70:
                ...
                logger.info(
                    f"  ✓ Merged '{name}' with existing '{best_key}' (score: {best_score})"
                )
...
def test_deduplicator():
    ...
    if len(deduplicator.recipes_db) < 5:
        logger.info(
            f"Only {len(deduplicator.recipes_db)} recipes available. Testing with all."
        )
