...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/to_do_sync.log"), logging.StreamHandler()],
)
...
class ToDoSync:
    ...
    def get_or_create_list(self, list_name: str) -> Optional[str]:
        try:
            response = requests.get(
                self.todo_endpoint, headers=self.get_headers(), timeout=10
            )
            response.raise_for_status()
            ...
            response = requests.post(
                self.todo_endpoint, json=body, headers=self.get_headers(), timeout=10
            )
            response.raise_for_status()
    ...
    def add_task(self, list_id: str, task_title: str, subtasks: list = None) -> bool:
        try:
            ...
            body = {"title": task_title, "categories": ["Menu"]}

            response = requests.post(
                endpoint, json=body, headers=self.get_headers(), timeout=10
            )
            response.raise_for_status()
    ...
    def sync_menu(self, menu: Dict) -> bool:
        ...
        for dinner in menu.get("dinners", []):
            ...
            ingredients = [
                f"- {ing['ingredient']}: {ing['quantity']} {ing['unit']}"
