...
with open("data/sample_recipes.json") as f:
    ...
    for recipe in recipes:
        if not ("id" in recipe or "recipe_id" in recipe):
            print("✗ Recipe missing id/recipe_id")
            exit(1)
        if not any(k in recipe for k in ["title", "title_en", "title_no"]):
            print("✗ Recipe missing title field")
            exit(1)
...
with open("pantry_staples.json") as f:
    ...
    if "pantry_staples_english" in pantry and "pantry_staples_norwegian" in pantry:
        print(
            f"✓ Pantry staples has {len(pantry['pantry_staples_english'])} English items"
        )
        print(
            f"✓ Pantry staples has {len(pantry['pantry_staples_norwegian'])} Norwegian items"
        )
    else:
