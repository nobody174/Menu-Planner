...
class User(Base):
    ...
    referred_by_code = Column(
        String(12), nullable=True
    )  # raw code, survives referrer deletion
...
class Household(Base):
    ...
    menus = relationship(
        "WeeklyMenu", back_populates="household", cascade="all, delete-orphan"
    )
...
class HouseholdMember(Base):
    ...
    )  # shown for profiles instead of User.email
    avatar_type = Column(String(20), nullable=True)  # 'preset', 'upload', or NULL
    avatar_value = Column(
        String(500), nullable=True
    )  # preset filename or uploaded image path/URL
